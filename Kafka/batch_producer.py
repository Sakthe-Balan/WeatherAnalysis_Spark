...
def batch_producer(bootstrap_servers='localhost:9092', topic='weather_data_batch', batch_size=500):
    ...
    mysql_cursor.execute("SELECT * FROM weather_data")
    ...
    while rows:
        ...
        producer.flush()
        print(f"Batch of {len(batch_data)} rows sent to Kafka topic '{topic}'")
